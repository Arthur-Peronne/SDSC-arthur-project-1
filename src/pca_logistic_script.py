...
# PARAMETERS
# 1 / Global parameters
Y_name = "weight" # "group", "height", "weight"
group_binYN = False
...
imageROIonlyYN = True
pca_folder = "pca_logisticreg/"
cumvar_threshold = 1.0 # 1.0, 0.99, 0.95, 0.9, 0.85, 0.8, 0.7, 0.6, 0.4
cumvar_threshold_list = [2.0, 0.99, 0.98, 0.965, 0.95, 0.92, 0.9, 0.87, 0.85, 0.82, 0.8, 0.75, 0.7, 0.68, 0.65, 0.5, 0.4] # For PCA on voxROI images
...
n_idealpc_confusion = 14 # 14 for voxROI, 5 for mask binary
n_idealpc_regression = 7

# # Load data: X and Y
# X_train, X_test, Y_train, Y_test  = plf.load_xy(source_folder, pca_folder, pca_description, maskYN, maskbinYN, imageROIonlyYN, Y_name, group_binYN, group_binvalue, recalculate= False)

# # PCA 
# pca, X_train_pca, meta = pef.pca_patients(X_train, pca_folder, pca_description, normalize_rows=not maskbinYN, recalculate = False)
# X_test_pca = pca.transform(X_test)
# n_pc_tokeep = plf.n_pc_for_variance(pca, cumvar_threshold)
# # n_pc_tokeep = n_idealpc_regression
...
# # Logistic regression 
# clf = plf.logisticreg(X_train_pca_n, Y_train, pca_folder, pca_description, Y_name, group_binvalue, n_pc_tokeep, multi_class = not group_binYN, recalculate=True)
...
# # Linear regression 
# reg = plf.linearreg(X_train_pca_n, Y_train, pca_folder, pca_description, Y_name, n_pc_tokeep, recalculate=True)
...
if group_binYN: # binary regression
    ...
    plf.plot_prediction_results_mtc(pca_folder, pca_description, Y_name)
    plf.plot_confusion_matrix_mtc(pca_folder, pca_description, Y_name, n_idealpc_confusion)
